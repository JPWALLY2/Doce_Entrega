...
@section('content_header')
     ...
     <form method="POST"
           class="form-inline" 
           action="{{ route('produtosforn.pesq') }}">
       {{ csrf_field() }}
      ...
      <a href="{{ route('produtosforn.index') }}" 
         class="btn btn-warning" role="button">
          Todos</a>      
     </form>
        </div>
    <h1>
    <a href="{{ route('produtosforn.create') }}" 
...
@forelse($produtosforn as $p)
  ...
  @if ($loop->iteration == $loop->count && $acao == 1)
    <tr><td colspan=9>Nº de Produtos cadastrados : {{ $numProdforn}}
        </td></tr> 
  @endif
